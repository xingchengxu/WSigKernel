...
try:
    with open('trained_models_factorial.pkl', 'rb') as file:
...
for name in datasets:
    ...
    for (at, ll) in transforms:
        ...
        G_train = wsigkernel.sig_kernel_matrix_wt(x_train, x_train, n=0, solver=0, sym=True, rbf=False, sigma=1., weight=1)
    ...
    # save trained models
    with open('trained_models_factorial.pkl', 'wb') as file:
...
try:
    with open('final_results_factorial.pkl', 'rb') as file:
...
for name in _datasets:
    for ker in _kernels:
        ...
        # G_test = sigkernel.sig_kernel_matrix(x_test, x_train, n=0, solver=0, sym=False, rbf=False)
        G_test = wsigkernel.sig_kernel_matrix_wt(x_test, x_train, n=0, solver=0, sym=False, rbf=False, sigma=1.,
                                                 weight=1)
        ...
        # save results
        with open('final_results_factorial.pkl', 'wb') as file:
